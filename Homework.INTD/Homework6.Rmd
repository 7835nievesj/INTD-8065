...
cor(tempMortality)
```
The estimated correlations show compatible results with the graphphics, where Tempertature and Mortality rate seem to have a srtong association. 

###2. Fit the regression

```{r}
morT.lmod1 <- lm(Mortality.rate~Temperature, 
                data = tempMortality)
names(morT.lmod1)
coef(morT.lmod1)
```
